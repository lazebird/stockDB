...
def stock_update(tradedate: TradeDate, l: list, interval, start_date: datetime.date, end_date: datetime.date, force=False):
    ...
    for i, e in enumerate(l):
        ...
        if not tradedate.stock_check(s, start_date):
            Logger().err(f"[{i+1}/{max}] stock {s.code} {s.name} skipped, cause: not trade in {start_date}")
            continue
        ...
        for j, d in enumerate(datas):
            ...
            if not force and load_data(file, {}) != {}:
                Logger().info(f"[{i+1}/{max}][{j+1}/{dlen}] stock {s.code} {s.name} skipped, cause: {file} already exists")
                continue
            write_data(d, file)
            Logger().info(f"[{i+1}/{max}][{j+1}/{dlen}] updating file {file}, data {d}")
